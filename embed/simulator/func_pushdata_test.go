...
func TestGetPushData(t *testing.T) {
	...
	if !ok || iRet != 0 {
		t.Error("embed getPushData PUSH0 execute failed.")
	}
	...
	if !ok || !bytes.Equal(ret, []byte{1, 1, 1, 1}) {
		t.Fatal("embed getPushData PUSHDATA1 execute failed.")
	}
	...
	ret, ok = getPushData(&e).([]byte)
	if !ok || !bytes.Equal(ret, []byte{1, 1, 1, 1}) {
		t.Fatal("embed getPushData PUSHDATA2 execute failed.")
	}
	...
	ret, ok = getPushData(&e).([]byte)
	if !ok || !bytes.Equal(ret, []byte{1, 1, 1, 1}) {
		t.Fatal("embed getPushData PUSHDATA4 execute failed.")
	}
	...
	for _, opCode := range []OpCode{PUSHM1, PUSH1, PUSH2, PUSH3, PUSH4, PUSH5, PUSH6, PUSH7,
		...
		iRet, ok = getPushData(&e).(int8)
		if !ok || expect != iRet {
			t.Fatal("embed getPushData execute failed.")
		}
	}
}

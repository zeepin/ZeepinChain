...
zeepin计划支持多种共识算法，并提供可插拔式的共识切换机制。目前已支持的共识算法为GBFT与DBFT，这两种算法有不同的配置方法，主要体现在配置文件里。
现对配置文件的内容和结构解释如下。
...
```
SeedList：用来配置zeepin网络的种子节点。种子节点是zeepin网络的链接入口，新节点加入zeepin网络时，会先向种子节点请求网络相关信息。
配置文件里至少需配置一个种子节点。

ConsensusType：共识模式，指示该配置文件配置的是何种共识，目前支持"GBFT"和"DBFT"。
...
GBFT是目前zeepin的默认共识机制，配置文件为[`config-gbft.json`](config-gbft.json)，如下：

...
zeepin cli 是zeepin命令行客户端，用于启动和管理zeepin节点，管理钱包账户，发送交易以及部署和调用智能合约等。

* [zeepin cli 使用说明](#zeepin-cli-使用说明)
	* [1、启动和管理ZeepinChaiin节点](#1启动和管理zeepin节点)
		* [1.1 启动参数](#11-启动参数)
			* [1.1.1 zeepin 系统参数](#111-zeepin-系统参数)
			* [1.1.2 账户参数](#112-账户参数)
			* [1.1.3 共识参数](#113-共识参数)
		...
		* [1.2 节点部署](#12-节点部署)
			* [1.2.1 创世区块配置文件](#121-创世区块配置文件)
				* [1.2.1.1 GBFT配置文件](#1211-gbft配置文件)
	...
	* [3、资产管理](#3资产管理)
		...
		* [3.6 查看未解绑的GALA余额](#36-查看未解绑的gala余额)
		* [3.7 提取解绑的GALA](#37-提取解绑的gala)
			* [3.7.1 提取解绑的GALA参数](#371-提取解绑的gala参数)
	* [4、查询信息](#4查询信息)
...
zeepin cli有很多启动参数，用于配置很管理zeepin节点的一些行为。如果不带任何参数启动zeepin cli时，默认会作为一个同步节点接入zeepin的主网。

```
./zeepin
```
使用./zeepin -help 可以查看到zeepin cli节点支持的所有启动参数。

### 1.1 启动参数

以下是zeepin cli 支持的命令行参数：

#### 1.1.1 zeepin 系统参数

--config
config 参数用于指定当前zeepin节点创世区块配置文件的路径。如果不指定，将使用Polaris测试网的创世块配置。注意，同一个网络所有节点的创世区块配置必须一致，否则会因为区块数据不兼容导致无法启动节点或同步区块数据。

--loglevel
loglevel 参数用于设置zeepin输出的日志级别。zeepin支持从0:Debug 1:Info 2:Warn 3:Error 4:Fatal 5:Trace 6:MaxLevel 的7级日志，日志等级由低到高，输出的日志量由多到少。默认值是1，即只输出info级及其之上级别的日志。

--disableeventlog
disableeventlog 参数用于关闭智能合约执行时输出的event log，以提升节点交易执行性能。zeepin 节点默认会开启智能合约执行时的event log输出功能。

--datadir
datadir 参数用于指定区块数据的存放目录。默认值为"./Chain"。

--import
import 参数用于启动zeepin节点的区块导入功能，通过导入本地文件的方式来提高区块同步速度。
...
--wallet, -w
wallet 参数用于指定zeepin节点启动时的钱包文件路径。默认值为"./wallet.dat"。

--account, -a
account 参数用于指定zeepin节点启动时的账户地址。不填则使用钱包默认账户。

--password, -p
password 参数用于指定zeepin节点启动的账户密码。因为在命令行中输入的账户密码会被保存在系统的日志中，容易造成密码泄露，因此在生产环境中建议不要使用该参数。
...
--disablerpc
disablerpc 参数用于关闭rpc服务器。zeepin节点在启动时会默认启动rpc服务器。
...
disabletxpoolpreexec 参数用于关闭交易池中对来自网络的交易预执行校验。zeepin节点在启动时交易池默认打开预执行。

--disablesyncverifytx
disablesyncverifytx 参数用于关闭rpc、restful、websocket中同步验证交易

--enablebroadcastnettx
enablebroadcastnettx 参数用于打开交易池广播来自网络的交易。zeepin节点在启动时交易池默认关闭广播来自网络的交易功能的。

### 1.2 节点部署

#### 1.2.1 创世区块配置文件

zeepin支持GBFT和dBFT共识算法，一个区块链网络必须使用同一种共识算法，否则节点将无法正常运行。zeepin内置了Polaris测试网的配置，如果启动zeepin时不指定创世块配置文件，将会使用Polaris测试网络的创世块配置；如果需要搭建zeepin私有链，则需要通过--config指定创世块配置文件的路径。

使用dBFT共识算法部署zeepin测试网络最少需要4个节点，使用GBFT共识算法部署zeepin测试网络最少需要7个节点。

##### 1.2.1.1 GBFT配置文件

使用GBFT共识算法的创世块配置文件示例：
...
```
{
  ...
  "ConsensusType":"gbft",           //指定当前网络使用GBFT算法
  "GBFT":{                          //GBFT算法配置
    "n":9,                          //网络规模（暂时未使用）
    "c":3,                          //容错节点数量
    "k":9,                          //共识节点数量
...
按照角色不同，节点可以分为记账节点和同步节点，记账节点参与网络共识，而同步节点只同步记账节点生成的区块。由于zeepin默认是不启动共识模块的，因此部署记账节点需要通过--enableconsensus命令行参数开启共。此外，zeepin节点默认会启动Rpc服务器，同时会输出智能合约输出的Event Log，因此如果没有特殊要求，可以使用--disablerpc和--disableeventlog命令行参数关闭rpc和eventlog模块。

推荐记账节点启动参数：

```
./zeepin --enableconsensus --disablerpc --disableeventlog
```
...
可以在同一台机器上部署zeepin的测试网络，本质上与在多台机器上部署多节点没有什么不同，但是注意使用不同的端口号：

```
./zeepin --enableconsensus --nodeport=XXX --rpcport=XXX
```
#### 1.2.5 单节点测试网络部署

zeepin支持单节点网络部署，用于开发测试环境搭建。启动单节点测试网络只需要加上--testmode参数即可。
...
使用 ./zeepin account --help 命令可以查看钱包管理命令的帮助信息。

### 2.1、添加账户

zeepin支持多种加密算法，包括ECDSA、SM2以及ED25519。
...
每个钱包都一个默认账户，一般情况下是第一个添加的账户。默认账户不能被删除，可以通过./zeepin account set 命令来修改默认账户。
...
--gid
gid参数用来创建GID，而不是普通账户。

**添加账户**

```
./zeepin account add --default
```

通过 ./zeepin account add --help 可以查看帮助信息。
...
在zeepin cli中，可以通过Index、Address或非空的Label来查找账户。

使用--v 可以查看账户的详细信息。
通过 ./zeepin account list --help 可以查看帮助信息。

### 2.3 修改账户

使用修改账户命令可以修改账户的标签，重新设置默认账户，修改账户密码，如果账户是ECDSA加密算法的密钥，还可以修改密钥的签名方案。
通过 ./zeepin account set --help 可以查看帮助信息。
...
```
./zeepin account set --d <address|index|label>
```
**修改账户标签**

```
./zeepin account set --label=XXX <address|index|label>
```
**修改账户密码**

```
./zeepin account set --changepasswd <address|index|label>
```

**修改ECDSA密钥签名方案**

```
./zeepin account set --s=SHA256withECDSA <address|index|label>
```
### 2.4 删除账户
...
```
/zeepin account del <address|index|label>
```
### 2.5 导入账户
...
```
./zeepin account import -s=./source_wallet.dat
```

#### 2.5.2 通过WIF导入账户
获得WIF并把WIF存入key.txt文件，并通过以下命令导入
zeepin account import --wif --source key.txt

## 3、资产管理

资产管理命令可以查看账户的余额，执行ZPT/GALA转账，提取GALA以及查看未绑定的GALA等操作。

### 3.1 查看账户余额

```
./zeepin asset balance <address|index|label>
```
### 3.2 ZPT/GALA转账
...
--gaslimit
gaslimit参数指定转账交易的gas limit。交易的gas limit不能小于接收节点交易池设置的最低gas limit，否则交易会被拒绝。gasprice * gaslimit 为账户实际支付的GALA 费用。 默认值为20000。

--asset
asset参数指定转账的资产类型，zpt表示ZPT，gala表示GALA。默认值为zpt。
...
--amount
amount参数指定转账金额。注意：由于ZPT的精度是1，因此如果输入的是个浮点值，那么小数部分的值会被丢弃；GALA的精度为9，因此超出9位的小数部分将会被丢弃。

**转账**

```
./zeepin asset transfer --from=<address|index|label> --to=<address|index|label> --amount=XXX --asset=zpt
```
...
--amount
amount参数指定授权转账金额。注意：由于ZPT的精度是1，因此如果输入的是个浮点值，那么小数部分的值会被丢弃；GALA的精度为9，因此超出9位的小数部分将会被丢弃。

**授权转账**

```
./zeepin asset approve --from=<address|index|label> --to=<address|index|label> --amount=XXX --asset=zpt
```
...
```
./zeepin asset allowance --from=<address|index|label> --to=<address|index|label>
```
...
--amount
amount参数指定转账金额，转账金额不能大于授权转账余额，否则交易会执行失败。注意：由于ZPT的精度是1，因此如果输入的是个浮点值，那么小数部分的值会被丢弃；GALA的精度为9，因此超出9位的小数部分将会被丢弃。

**从授权账户中转账**

```
./zeepin asset transferfrom --from=<address|index|label> --to=<address|index|label> --sender=<address|index|label> --amount=XXX
```

### 3.6 查看未解绑的GALA余额

GALA采用定时解绑策略解除绑定在ZPT上的GALA。使用如下命令可以查看到当前账户未解绑的GALA余额。

```
./zeepin asset unboundgala <address|index|label>
```
### 3.7 提取解绑的GALA

使用提取命令可以提取当前所有未解绑的GALA。
...
**提取解绑的GALA**
```
./zeepin asset withdrawgala <address|index|label>
```
## 4、查询信息

查询信息命令可以查询区块、交易以及交易执行等信息。使用./zeepin info block --help 命令可以查看帮助信息。

### 4.1 查询区块信息

```
./zeepin info block <height|blockHash>
```
...
```
其中，State表示交易执行结果，State的值为1，表示交易执行成功，State值为0时，表示执行失败。GasConsumed表示交易执行消耗的GALA。Notify表示交易执行时输出的Event log。不同的交易可能会输出不同的Event log。
...
--gaslimit
gaslimit参数指定部署合约交易的gas limit。交易的gas limit不能小于接收节点交易池设置的最低gas limit，否则交易会被拒绝。gasprice * gaslimit 为账户实际支付的GALA 费用。

**对于合约部署，gaslimit 值必须大于20000000，同时账户中必须保有足够的GALA余额。**
...
--prepare, -p
prepare参数用于预部署合约, 预部署不会把合约部署到zeepin上， 也不会消耗人任何GALA。通过预部署合约，用户可以知道当前合约部署所需要消耗的gas limit。

**智能合约部署**

```
./zeepin contract deploy --name=xxx --code=xxx --author=xxx --desc=xxx --email=xxx --needstore --gaslimit=100000000
```
...
可以通过 ./zeepin info status <TxHash> 命令查询合约执行状态。如果返回错误如：UNKNOWN TRANSACTION时，表示交易没有落帐，有可能交易还在交易池中排队等待被打包，也有可能表示交易因为gaslimit或者时gasprice设置过低，导致交易被拒绝。
...
--prepare, -p
prepare参数表示当前为预执行，执行交易不会被打包到区块中，也不会消耗任何GALA。预执行会返回合约方法的返回值，同时还会试算当前调用需要的gas limit。
...
```
./zeepin contract invoke --address=XXX --params=XXX --return=XXX --p
```
返回示例：
...
```
./zeepin contract invoke --address=XXX --params=XXX --gaslimit=XXX
```

智能合约在执行之前，可以通过预执行，试算出当前执行所需要的gas limit，以避免GALA余额不足导致执行失败。
...
```
./zeepin contract invokeCode --code=XXX --gaslimit=XXX
```

## 6、区块导入导出

zeepin Cli支持导出本地节点的区块数据到一个压缩文件中，生成的压缩文件可以再导入其它zeepin节点中。出于安全考虑，导入的区块数据文件请确保是从可信的来源获取的。

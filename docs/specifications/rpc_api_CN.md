...
本文档是zeepin的RPC接口文档，详细定义了每种接口的参数与返回值。
...
| [getversion](#10-getversion) |  | 得到运行的zeepin版本 |  |
...
| [getunboundgala](#20-getunboundgala) | address | 返回该账户未提取的gala |  |
...
Hex: 签名后的交易序列化成的十六进数据。可以参考zeepin-go-sdk/rpc.go中的NewNativeInvokeTransaction方法生成。
...
```
    ...
    rpcClient := sdk.Rpc
    // 生成native合约调用交易; 如果想调用WASM VM合约，可以使用NewWASMVMSInvokeTransaction方法

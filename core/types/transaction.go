...
func (self *Transaction) GetSignatureAddresses() []common.Address {
	...
	//log.Debugf("GetSignatureAddresses: len self.Sigs: %d", len(self.Sigs))
	for _, sig := range self.Sigs {
		m := int(sig.M)
		n := len(sig.PubKeys)
		//log.Debugf("GetSignatureAddresses: m: %d, n:%d", m, n)

		if n == 1 {
			address = append(address, AddressFromPubKey(sig.PubKeys[0]))
		} else {
			addr, err := AddressFromMultiPubKeys(sig.PubKeys, m)
			/*for i := range sig.PubKeys {
				pk := AddressFromPubKey(sig.PubKeys[i])
				log.Debugf("GetSignatureAddresses: addr: %s, sig: %s, m: %d, n:%d", addr, pk.ToBase58(), m, n)
			}*/
			if err != nil {
				return nil
			}

...
import (
	...
	"github.com/mileschao/ZeepinChain/smartcontract/service/native/global_params"
	...
	sstate "github.com/mileschao/ZeepinChain/smartcontract/states"
...
func (this *LedgerStoreImp) verifyHeader(header *types.Header, vbftPeerInfo map[string]uint32) (map[string]uint32, error) {
	...
	if consensusType == "gbft" {
		//check bookkeeppers
		m := int(math.Ceil(float64(len(header.Bookkeepers)) * 2.0 / 3.0))
		if len(header.Bookkeepers) < m {
			return vbftPeerInfo, fmt.Errorf("header Bookkeepers %d less than 2/3 len gbftPeerInfo%d", len(header.Bookkeepers), len(vbftPeerInfo))
		}

...
zeepin is dedicated to creating a modularized, freely configurable, interoperable cross-chain, high-performance, and horizontally scalable blockchain infrastructure system. zeepin makes deploying and invoking decentralized applications easier.
...
The public test network is described below. We sincerely welcome and hope more developers join zeepin.
...
- Multiple consensus algorithm support (GBFT/DBFT/RBFT/SBFT/PoW)
...
- [Server deployment](#server-deployment)
    ...
    - [ZPT transfer sample](#zpt-transfer-sample)
...
- `tools/sigsvr`: (optional)zeepin Signature Server - sigsvr is a rpc server for signing transactions for some special requirement.detail docs can be reference at [link](./docs/specifications/sigsvr.md)
...
- Node program + Node control program  `zeepin`
- Wallet file`wallet.dat`

Run command `$ ./zeepin --testmode --networkid 3` can start single-host test net.
...
Note: When you want to build a private net to run zeepin in DBFT or GBFT, you must use --config argument to specify a configuration file, and
...
In the multi-hosts enviroment, we need 9 nodes to run zeepin at least in GBFT.

We can perform a quick deployment by modifying the configuration file [`config-gbft.json`](./docs/specifications/config-gbft.json), 
click [here](./docs/specifications/config.md) to read instruction of config file.

1. Generate 9 wallet file, each wallet contains an account. These account is bookkeepers of consensus. The account generated by :
	```
...
2. Modify `config-gbft.json`, set public key and address of 9 accounts generated in last step into peers config in `config-gbft.json`.
...
5. Seed nodes configuration
   ...
   - Select at least one seed node out of 9 hosts and fill the seed node address into the `SeelList` of each configuration file. The format is `Seed node IP address + Seed node NodePort`.

##### DBFT Deployment

In the multi-hosts enviroment, we need 4 nodes to run zeepin at least in DBFT.
...
Run `./zeepin --help` for details, also you can read [zeepin CLI user guide](./docs/specifications/cli_user_guide.md) to get more information.
...
1. make docker image
    ...
    - In the root directory of source code，run`make docker`, it will make zeepin image in docker.

2. run zeepin image

    - Use command `docker run zeepin/zeepin`to run zeepin；

    - If you need to allow interactive keyboard input while the image is running, you can use the `docker -ti run zeepin/zeepin` command to start the image;
    ...
    - If you need to add zeepin parameters, you can add them directly after `docker run zeepin/zeepin` such as `docker run zeepin/zeepin --networkid 2`.
     The parameters of zeepin command line refer to [here](./docs/specifications/cli_user_guide.md).

### ZPT transfer sample
 -- from: transfer from； -- to: transfer to； -- amount: zpt amount；
```shell
  ./zeepin asset transfer  --to=AXkDGfr9thEqWmCKpTtQYaazJRwQzH48eC --amount=10
```
...
```shell
./zeepin asset balance --address=AYiToLDT2yZuNs3PZieXcdTpyC5VWQmfaN
```
result：
```shell
...
Please open a pull request with a signed commit. We appreciate your help! You can also send your code as emails to the developer mailing list. You're welcome to join the zeepin mailing list or developer forum.
...
The zeepin library is licensed under the GNU Lesser General Public License v3.0, read the LICENSE file in the root directory of the project for details.

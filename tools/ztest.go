...
func main() {
	...
	flag.StringVar(&address, "address", "", "wallet address")
	flag.Parse()
	...
	case "getPeerpoolInfo":
		...
		return
	case "getVoteInfo":
		voteAddress, err := common.AddressFromBase58(address)
		if err != nil {
			fmt.Println("get address fail:", err)
			return
		}
		params := &governance.GetVoteInfoParam{
			PeerPubkey: publicKey,
			Address:    voteAddress,
		}
		preresult, err := PrepareInvokeNativeContract(nutils.GovernanceContractAddress, 0, "getVoteInfo", []interface{}{params})
		if err != nil {
			fmt.Println("get PeerpoolInfo fail:", err)
			return
		}
		...
		voteInfobytes, _ := common.HexToBytes(preresult.Result.(string))
		fmt.Println(preresult.Result)
		if err := voteInfo.Deserialize(bytes.NewBuffer(voteInfobytes)); err != nil {
			fmt.Println("deserialize, deserialize peerPoolMap error!", err)
		}
		fmt.Printf("VoteInfo :%+v \n", voteInfo)
		return
	case "peersInfo":
	...
	case "withdraw":
		...
		code, err := BuildNativeInvokeCode(nutils.GovernanceContractAddress, 0, "withdraw", []interface{}{params})
		if err != nil {
			fmt.Println("build native invoke code error:", err)
			return
		}
	...
	case "checkVoteInfo":
		params := &governance.CheckVoteInfoParam{
			PeerPubkey: publicKey,
		}
		code, err := BuildNativeInvokeCode(nutils.GovernanceContractAddress, 0, "checkVoteInfo", []interface{}{params})
		if err != nil {
			fmt.Println("build native invoke code error:", err)
			return
		}
...
func AddressPrint(s string) {
	addr, err := common.AddressFromBase58(s)
	if err != nil {
		fmt.Printf("address print %s", err)
		return
	}
...
func sendRpcRequest(method string, params []interface{}) ([]byte, error) {
	...
	if rpcRsp.Error != 0 {
		res, _ := rpcRsp.Result.MarshalJSON()
		return nil, fmt.Errorf("error code:%d desc:%s, result: %s", rpcRsp.Error, rpcRsp.Desc, string(res))
	}

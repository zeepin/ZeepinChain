...
import (
	...
	"github.com/mileschao/ZeepinChain/cmd"
	...
	"github.com/mileschao/ZeepinChain/consensus"
	...
	hserver "github.com/mileschao/ZeepinChain/http/base/actor"
	"github.com/mileschao/ZeepinChain/http/jsonrpc"
	"github.com/mileschao/ZeepinChain/http/localrpc"
	"github.com/mileschao/ZeepinChain/http/nodeinfo"
	"github.com/mileschao/ZeepinChain/http/restful"
	"github.com/mileschao/ZeepinChain/http/websocket"
	"github.com/mileschao/ZeepinChain/p2pserver"
	netreqactor "github.com/mileschao/ZeepinChain/p2pserver/actor/req"
	p2pactor "github.com/mileschao/ZeepinChain/p2pserver/actor/server"
	"github.com/mileschao/ZeepinChain/txnpool"
	tc "github.com/mileschao/ZeepinChain/txnpool/common"
	"github.com/mileschao/ZeepinChain/txnpool/proc"
...
func setupAPP() *cli.App {
	app := cli.NewApp()
	app.Usage = "zeepin CLI"
	app.Action = startZeepinChain
...
func startZeepinChain(ctx *cli.Context) {
	initLog(ctx)
...
func initConfig(ctx *cli.Context) (*config.ZeepinChainConfig, error) {
	//init ZeepinChain config from cli
	cfg, err := cmd.SetZeepinChainConfig(ctx)
	if err != nil {
		return nil, err
	}
...
func waitToExit() {
	...
	go func() {
		for sig := range sc {
			log.Infof("zeepin received exit signal:%v.", sig.String())
			close(exit)
			break
		}
	}()

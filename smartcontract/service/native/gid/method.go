...
func regIdWithPublicKey(srvc *native.NativeService) ([]byte, error) {
	...
	if err != nil {
		return utils.BYTE_FALSE, errors.New("register GID error: parsing argument 0 failed")
	} else if len(arg0) == 0 {
		return utils.BYTE_FALSE, errors.New("register GID error: invalid length of argument 0")
	}
	// arg1: public key
	arg1, err := serialization.ReadVarBytes(args)
	if err != nil {
		return utils.BYTE_FALSE, errors.New("register GID error: parsing argument 1 failed")
	}
	...
	if len(arg0) == 0 || len(arg1) == 0 {
		return utils.BYTE_FALSE, errors.New("register GID error: invalid argument")
	}

	if !account.VerifyID(string(arg0)) {
		return utils.BYTE_FALSE, errors.New("register GID error: invalid ID")
	}

	key, err := encodeID(arg0)
	if err != nil {
		return utils.BYTE_FALSE, errors.New("register GID error: " + err.Error())
	}

	if checkIDExistence(srvc, key) {
		return utils.BYTE_FALSE, errors.New("register GID error: already registered")
	}

	public, err := keypair.DeserializePublicKey(arg1)
	if err != nil {
		log.Error(err)
		return utils.BYTE_FALSE, errors.New("register GID error: invalid public key")
	}
	addr := types.AddressFromPubKey(public)
	if !srvc.ContextRef.CheckWitness(addr) {
		return utils.BYTE_FALSE, errors.New("register GID error: checking witness failed")
	}

	// insert public key
	_, err = insertPk(srvc, key, arg1)
	if err != nil {
		return utils.BYTE_FALSE, errors.New("register GID error: store public key error, " + err.Error())
	}
	// set flags

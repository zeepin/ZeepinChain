 ...
 */
func initContractAdmin(native *native.NativeService, contractAddr common.Address, GID []byte) (bool, error) {
	...
	err = putContractAdmin(native, contractAddr, GID)
	if err != nil {
		return false, err
	}
	return true, nil
...
func InitContractAdmin(native *native.NativeService) ([]byte, error) {
	...
	if !account.VerifyID(string(param.AdminGID)) {
		return nil, fmt.Errorf("[initContractAdmin] invalid param: adminGID is %x", param.AdminGID)
	}
	ret, err := initContractAdmin(native, invokeAddr, param.AdminGID)
	if err != nil {
	...
	msg := []interface{}{"initContractAdmin", invokeAddr.ToHexString(), string(param.AdminGID)}
	pushEvent(native, msg)
	return utils.BYTE_TRUE, nil
}

func transfer(native *native.NativeService, contractAddr common.Address, newAdminGID []byte, keyNo uint64) (bool, error) {
	admin, err := getContractAdmin(native, contractAddr)
	if err != nil {
	...
	utils.PutBytes(native, adminKey, newAdminGID)
	return true, nil
}
...
func Transfer(native *native.NativeService) ([]byte, error) {
	...
	if !account.VerifyID(string(param.NewAdminGID)) {
		return nil, fmt.Errorf("[transfer] invalid param: newAdminGID is %x", param.NewAdminGID)
	}
	//prepare event msg
	...
	//call transfer func
	ret, err := transfer(native, param.ContractAddr, param.NewAdminGID, param.KeyNo)
	if err != nil {
...
func AssignFuncsToRole(native *native.NativeService) ([]byte, error) {
	...
	if bytes.Compare(admin, param.AdminGID) != 0 {
		log.Debugf("[assignFuncsToRole] invalid param: adminGID doesn't match %s != %s",
			string(param.AdminGID), string(admin))
		pushEvent(native, failState)
		return utils.BYTE_FALSE, nil
	}
	ret, err := verifySig(native, param.AdminGID, param.KeyNo)
	if err != nil {
		return nil, fmt.Errorf("[assignFuncsToRole] verify admin's signature failed: %v", err)
	}
	if !ret {
		log.Debugf("[assignFuncsToRole] verifySig return false: adminGID=%s, keyNo=%d",
...
func assignToRole(native *native.NativeService, param *GIDsToRoleParam) (bool, error) {
	...
	if bytes.Compare(admin, param.AdminGID) != 0 {
		log.Debugf("param's adminGID doesn't match: %s != %s", string(param.AdminGID),
			string(admin))
		return false, nil
	}
	valid, err := verifySig(native, param.AdminGID, param.KeyNo)
	if err != nil {
		return false, fmt.Errorf("verify admin's signature failed: %v", err)
	}
	if !valid {
		log.Debugf("[assignGIDsToRole] verifySig return false: adminGID=%s, keyNo=%d",
			string(admin), param.KeyNo)
		return false, nil
	}
	...
	for _, p := range param.Persons {
		...
		tokens, err := getGIDToken(native, param.ContractAddr, p)
		if err != nil {
			return false, fmt.Errorf("getGIDToken failed: %v", err)
		}
		if tokens == nil {
		...
		err = putGIDToken(native, param.ContractAddr, p, tokens)
		if err != nil {
			return false, err
		}
	}
...
func AssignGIDsToRole(native *native.NativeService) ([]byte, error) {
	//deserialize param
	param := new(GIDsToRoleParam)
	rd := bytes.NewReader(native.Input)
	if err := param.Deserialize(rd); err != nil {
		return nil, fmt.Errorf("[assignGIDsToRole] deserialize param failed: %v", err)
	}

	if param.Role == nil {
		return nil, fmt.Errorf("[assignGIDsToRole] invalid param: role is nil")
	}
	for i, GID := range param.Persons {
		if !account.VerifyID(string(GID)) {
			return nil, fmt.Errorf("[assignGIDsToRole] invalid param: param.Persons[%d]=%s",
				i, string(GID))
		}
	}
	...
	if err != nil {
		return nil, fmt.Errorf("[assignGIDsToRole] failed: %v", err)
	}

	contract := param.ContractAddr.ToHexString()
	failState := []interface{}{"assignGIDsToRole", contract, false}
	sucState := []interface{}{"assignGIDsToRole", contract, true}
	if ret {
		pushEvent(native, sucState)
		return utils.BYTE_TRUE, nil
	} else {
...
func getAuthToken(native *native.NativeService, contractAddr common.Address, GID, role []byte) (*AuthToken, error) {
	tokens, err := getGIDToken(native, contractAddr, GID)
	if err != nil {
	...
	status, err := getDelegateStatus(native, contractAddr, GID)
	if err != nil {
...
func hasRole(native *native.NativeService, contractAddr common.Address, GID, role []byte) (bool, error) {
	token, err := getAuthToken(native, contractAddr, GID, role)
	if err != nil {
		return false, err
	}
	if token == nil {
...
func getLevel(native *native.NativeService, contractAddr common.Address, GID, role []byte) (uint8, error) {
	token, err := getAuthToken(native, contractAddr, GID, role)
	if err != nil {
		return 0, err
	}
...
func delegate(native *native.NativeService, contractAddr common.Address, from []byte, to []byte,
	...
	if !account.VerifyID(string(to)) {
		return false, fmt.Errorf("can not pass GID validity test: to=%s", string(to))
	}
...
func verifyToken(native *native.NativeService, contractAddr common.Address, caller []byte, fn string, keyNo uint64) (bool, error) {
	...
	tokens, err := getGIDToken(native, contractAddr, caller)
	if err != nil {
		return false, fmt.Errorf("getGIDToken failed: %v", err)
	}
	if tokens != nil {
...
func verifySig(native *native.NativeService, GID []byte, keyNo uint64) (bool, error) {
	bf := new(bytes.Buffer)
	if err := serialization.WriteVarBytes(bf, GID); err != nil {
		return false, err
	}
	...
	args := bf.Bytes()
	ret, err := native.NativeCall(utils.GIDContractAddress, "verifySignature", args)
	if err != nil {
		return false, err
	}
	valid, ok := ret.([]byte)
...
func RegisterAuthContract(native *native.NativeService) {
	...
	native.Register("assignGIDsToRole", AssignGIDsToRole)
